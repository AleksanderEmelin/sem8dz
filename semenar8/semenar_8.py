...
def file_read():
    with open("semenar8/phonebook.txt", "r", encoding="UTF-8") as file:
        return file.read()


def file_append(text=""):
    with open("semenar8/phonebook.txt", "a", encoding="UTF-8") as file:
        file.write(text)
...
# Удаление контакта
    ...
    for i, contact_str in enumerate(cont_list):
        cont_list = contact_str.replace('\n',' ').split()
        if search in cont_list:
            print(contact_str)
            print('Вы хотите  удалить контакт? (yes/no)?: ')
            name_del = input('> ')
            if name del == 'yes':
                del contacts_list[i]
...
# Редактирование контакта
    ...
    for i, contact_str in enumerate(cont_list):
        ...
        if search in cont_list:
            print(contact_str)
            print('Вы хотите  изменить контакт? (yes/no)?: ')
            name_del = input('> ')
            if name del == 'yes':
                sur = input_sur()
                name = input_name()
                ...
                adr = input_adr()
                contact_str = f"{sur} {name} {pat} {phone} {adr}\n\n"
                del contacts_list[i] = contact_str
    with open('phonebook.txt', 'w') as file:
        file_read()
            print('Вы изменили контакт   ')
